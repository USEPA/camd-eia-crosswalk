...
output_crosswalk <- function(epa_eia_crosswalk, only_mismatch = FALSE, agg_level = c("plant")) {
  ...
  if (!file.exists("data/outputs/epa_eia_crosswalk.xlsx")) {
    wb <- createWorkbook()
  } else {
    wb <- loadWorkbook("data/outputs/epa_eia_crosswalk.xlsx")
  ...
  saveWorkbook(wb, "data/outputs/epa_eia_crosswalk.xlsx", overwrite = TRUE)
  ...
  write_excel_csv(epa_eia_crosswalk,
                  "data/outputs/epa_eia_crosswalk.csv",
                  col_names = TRUE,
                  na = ""
  )
