...
save_output_data <- function(data, output_folder, filename){
  ...
  #' 
  #' @return Saves the RDS dataset in {output_folder}/outputs/{crosswalk_year}
  #'         directory
  #'         
  #' @examples 
  ...
  if(dir.exists(glue::glue("data/{output_folder}/outputs/{crosswalk_year}"))) {
    print(glue::glue("Folder {output_folder}/outputs/{crosswalk_year} already exists."))
  } else {
    dir.create(glue::glue("data/{output_folder}/outputs/{crosswalk_year}"))
  }
  
  print(glue::glue("Saving {filename} to folder data/{output_folder}/outputs/{crosswalk_year}"))
  
  # save file
  write_rds(data, glue::glue("data/{output_folder}/outputs/{crosswalk_year}/{filename}"))
  
  # check if file is successfully written to folder
  if(file.exists(glue::glue("data/{output_folder}/outputs/{crosswalk_year}/{filename}"))){
    print(glue::glue("File {filename} successfully written to folder data/{output_folder}/outputs/{crosswalk_year}"))
  } else {
