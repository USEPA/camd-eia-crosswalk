...
library(rvest) # Used to query all links on NEEDS page.
```

## Assign key values for data retrieval
```{r}
# API key for FACT API
api_key <- Sys.getenv("API_KEY_EIA")
eia_860_year <- 2021 # The  EIA data year (available at https://www.eia.gov/electricity/data/eia860/)
earliest_retirement_year <- 2010 # All units retired before this year are filtered out of the data

if (eia_860_year < 2021){
  eia_data_file <- str_glue("https://www.eia.gov/electricity/data/eia860/archive/xls/eia860{eia_860_year}.zip")
}else{
  eia_data_file <- str_glue("https://www.eia.gov/electricity/data/eia860/xls/eia860{eia_860_year}.zip")}
...
# include NEEDS IDs. By default, this will download the latest NEEDS data if the year used for analysis is not 2018.
include_NEEDS <- TRUE
...
if(!include_FRS) {
  ...
  camd_eia_crosswalk <- camd_eia_crosswalk %>%
  	left_join(frs, by = "CAMD_PLANT_ID") %>%
...
if(!include_NEEDS) {
  ...
  if (eia_860_year != 2018){ # Pull new data unless using 2018 data, which is already downloaded
    # Get reference case links matching year
    needs_url = "https://www.epa.gov/power-sector-modeling/national-electric-energy-data-system-needs"
    page = read_html(needs_url)
    links <- page %>% html_nodes("a") %>% html_attr("href") %>% str_subset(pattern = ".xlsx") %>% str_subset(regex("[R|r]eference[\\W|\\d]+[C|c]ase", TRUE)) %>% str_subset(pattern = as.character(eia_860_year)) 
    print(length(links))
    # If any, download
    if (length(links) != 0){
      path_to_file = str_glue("{wd}/data/needs_{eia_860_year}.xlsx")
      download.file(
    str_glue(paste("https://www.epa.gov", links[1], sep = "")),
    str_glue(path_to_file))
      
  }} else { # For 2018 data or if file not found
    path_to_file = str_glue("{wd}/data/needs_v6_november_2018_reference_case_0.xlsx")
  }    
  active_sheet = openxlsx::getSheetNames(path_to_file) %>% str_subset(pattern = regex("[A|a]ctive"))
    
  needs <-
    read_excel(
      str_glue(path_to_file),
      sheet = active_sheet[1],
      range = cell_cols("B:E")
    ) %>%
    select(
