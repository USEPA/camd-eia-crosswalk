...
1.  The EPA Plant ID and generator ID are matched to the EIA Plant ID and generator ID (from EIA-860 3_1\_Generator_Y{data year}) according to these steps:
...
2.  The EPA Plant ID, generator ID, and unit ID are matched to the EIA Plant ID, generator ID, and boiler ID (from EIA-860 6_1\_EnviroAssoc_Y{data year}) according to these steps:
...
> To add FRS IDs, the code calls the [FRS ID REST API](https://www.epa.gov/frs/frs-rest-services). These IDs are joined to EPA data via EPA_PLANT_ID. These IDs are at the facility level, so all subcomponents (generators and boilers) at one facility will be assigned the same FRS ID.
...
For more information on EPA's FACT API and to sign up for an API key, see <https://www.epa.gov/airmarkets/field-audit-checklist-tool-fact-api#/>.
